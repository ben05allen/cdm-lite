...
class ExecutionInstruction(BaseModel):
    product: NonTransferableProduct = Field(..., description="Defines the financial product to be executed and contract formed.")
    ...
    counterparty: list[cdm_base_staticdata_party_Counterparty_schema.Counterparty] | None = Field(
        None, description="Maps two defined parties to counterparty enums for the transacted product.", max_length=2, min_length=2
    )
    ...
    party_roles: list[cdm_base_staticdata_party_PartyRole_schema.PartyRole] | None = Field(
        None, alias="partyRoles", description="Defines the role(s) that party(ies) may have in relation to the execution.", min_length=0
    )
    execution_details: cdm_event_common_ExecutionDetails_schema.ExecutionDetails = Field(
        ..., alias="executionDetails", description="Specifies the type and venue of execution, e.g. via voice, or electronically."
    )
    ...
    trade_identifier: list[cdm_event_common_TradeIdentifier_schema.TradeIdentifier] | None = Field(
        None, alias="tradeIdentifier", description="Denotes one or more identifiers associated with the transaction.", min_length=1
    )
