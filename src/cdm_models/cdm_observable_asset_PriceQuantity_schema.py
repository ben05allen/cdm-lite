...
class PriceQuantity(BaseModel):
    price: list[cdm_observable_asset_metafields_FieldWithMetaPriceSchedule_schema.FieldWithMetaPriceSchedule] | None = Field(
        None, description="Specifies a price to be used for trade amounts and other purposes.", min_length=0
    )
    quantity: (
        list[cdm_base_math_metafields_FieldWithMetaNonNegativeQuantitySchedule_schema.FieldWithMetaNonNegativeQuantitySchedule] | None
    ) = Field(None, description="Specifies a quantity to be associated with an event, for example a trade amount.", min_length=0)
