# generated by datamodel-codegen:
#   filename:  cdm-base-staticdata-party-PartyRole.schema.json
#   timestamp: 2026-05-07T14:39:41+00:00

from pydantic import BaseModel, Field
from . import cdm_base_staticdata_party_PartyRoleEnum_schema, cdm_base_staticdata_party_metafields_ReferenceWithMetaParty_schema


class PartyRole(BaseModel):
    party_reference: cdm_base_staticdata_party_metafields_ReferenceWithMetaParty_schema.ReferenceWithMetaParty = Field(
        ..., alias="partyReference", description="A reference to the party to which the role refers to."
    )
    role: cdm_base_staticdata_party_PartyRoleEnum_schema.PartyRoleEnum = Field(..., description="The party role.")
    ownership_party_reference: cdm_base_staticdata_party_metafields_ReferenceWithMetaParty_schema.ReferenceWithMetaParty | None = Field(
        None,
        alias="ownershipPartyReference",
        description="A reference to the party that has ownership of this party role information. FpML specifies that For shared trade information, this attribute will reference the originator of the data (for example, an execution facility or clearing house).",
    )
